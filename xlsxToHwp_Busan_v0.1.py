...
def dragDrop(event):
    ...
    for file in sorted(sa_files):       # for문도 간단한 편
        # 중복 추가 방지
    ...
    # for files in sorted(sa_files):  # 수정: 'file'을 'files'로 변경
...
def start():
    if sa_list_file.size() != 1:
        msgbox.showwarning("경고", "'사전 협의 및 업데이트된 엑셀파일' 1개만 있어야 합니다!")
        return
...
# 가로 스크롤바
scrollbar_x = Scrollbar(sa_list_frame, orient=tk.HORIZONTAL)
scrollbar_x.pack(side="bottom", fill="x")

sa_list_file = Listbox(sa_list_frame, selectmode="extended", width = 15, height=3, yscrollcommand=scrollbar.set)
sa_list_file.pack(side="left", fill="both", expand=True)
scrollbar.config(command=sa_list_file.yview)
scrollbar_x.config(command=sa_list_file.xview)
